...
- (void)getAnnotationsFromURL:(NSURLRequest *)request success:(void (^)(NSArray *, id))success failure:(void (^)(NSError *))failure
{
    AFJSONRequestOperation* annotationOperation = [AFJSONRequestOperation 
                                                   JSONRequestOperationWithRequest:request 
                                                   success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
    ...
    NSOperationQueue* queue = [NSOperationQueue mainQueue];
    [queue addOperation:annotationOperation];
}
